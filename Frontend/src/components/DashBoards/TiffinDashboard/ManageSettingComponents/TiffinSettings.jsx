...
//         ? `${import.meta.env.VITE_SERVER_URL}/api/tiffin/email`
//         : `${import.meta.env.VITE_SERVER_URL}/api/add-tiffin`;
...
import { ClosureDaysSection } from "./ClosureDaysSection";
import { AdditionalSettingsSection } from "./AdditionalSettingsSection";
import { DeliveryTimeSlotsSection } from "./DeliveryTimeSlotsSection"; // Import the new component
import TiffinDetails from "./TiffinDetails";
import axios from "axios";
...
export default function TiffinSettings() {
  ...
  const [deliveryTimeSlots, setDeliveryTimeSlots] = useState([]); // New state for delivery slots
  ...
  useEffect(() => {
    ...
    const getTiffin = async () => {
      ...
      try {
        ...
        // Set delivery time slots from fetched data
    ...
    try {
      ...
      const response = await axios[method](
        endpoint,
        {
          ...
          operatingTimes: timings,
          additionalSettings: additionalSettings,
          deliveryTimeSlots: deliveryTimeSlots, // Include delivery time slots in the submission
        },
        { headers: { Authorization: `Bearer ${token}` }, withCredentials: true }
      );
    ...
    <div className="container mx-auto bg-gray-50 p-6">
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <ClosureDaysSection
          ...
          setClosureDates={setClosureDates}
        />
        {/* Integrate the new DeliveryTimeSlotsSection */}
        <DeliveryTimeSlotsSection
          deliveryTimeSlots={deliveryTimeSlots}
          setDeliveryTimeSlots={setDeliveryTimeSlots}
        />
        <AdditionalSettingsSection
          additionalSettings={additionalSettings}
          handleAdditionalSettingChange={handleAdditionalSettingChange}
