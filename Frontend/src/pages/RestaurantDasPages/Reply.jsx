...
import DisplayContentEmail from "./DisplayEmailContent";
...
function Reply({ email, mail, Replay,close1,replayType}) {
  ...
  const handleSentReply=()=>{
    ...
    console.log(emailId,replyContent);
    Axios.post(
      `${import.meta.env.VITE_SERVER_URL}/email/sent`,
      { emailId:emailId, subject:mail.subject, body:replyContent,preview:preview },
  ...
  const handleSubmitForward=()=>{
    ...
    Axios.post(
      `${import.meta.env.VITE_SERVER_URL}/email/sent`,
      { emailId:emailId, subject:mail.subject, body:replyContent,preview:mail.preview },
      { headers: { "Authorization": `Bearer ${token}` } }
    )
    .then((response) => {
      console.log("Mail sent successfully", response.data);
