...
const LeftNavbar = () => {
  ...
  const handleList=(name)=>{
    ...
    if(name==="Sent"){
      Axios.get(`${import.meta.env.VITE_SERVER_URL}/email/Sent?page=${currentPage}&limit=${limit}`,{headers:{"Authorization":`Bearer ${localStorage.getItem("token")}`}})
      .then((response)=>{
  ...
  const handleGetMails=()=>{
    ...
    Axios.get(`${import.meta.env.VITE_SERVER_URL}/email/inbox?page=${currentPage}&limit=${limit}`,{headers:{"Authorization":`Bearer ${localStorage.getItem("token")}`}})
    .then((response)=>{
      console.log(response.data)
  ...
  return (
    ...
    { name: "Drafts", icon: <MdDrafts className="h-5 w-5" /> },
      ...
      {openCompose &&<div
          className="fixed top-14 right-[150px]   w-[350px] h-[410px] transition-all duration-300 rounded-md"
        >
