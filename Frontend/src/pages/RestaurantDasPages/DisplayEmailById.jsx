...
const DisplayEmailById = ({ email, onBack, action, open,close1 }) => {
  ...
  const handleTrash=(emailId)=>{
    Axios.post(`${import.meta.env.VITE_SERVER_URL}/email/SentTrash`,{id:emailId},{headers:{"Authorization":`Bearer ${localStorage.getItem("token")}`}})
        .then((response)=>{
      ...
      <div className="i p-2">
        ...
        <div className="user p-2 m-1 flex items-center">
          ...
          <div className="info-user p-2 m-1 flex justify-between w-[90%] items-center">
            ...
            <h1 className="font-bold text-lg">{email.user.username} ( <span>{email.receiverMailId} )</span></h1>
