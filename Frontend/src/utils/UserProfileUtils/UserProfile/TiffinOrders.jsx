...
export const TiffinOrders = () => {
    ...
    const fetchOrders = useCallback(async (page) => {
        ...
        try {
            const response = await axios.get(
                `${import.meta.env.VITE_SERVER_URL}/api/orders/tiffin/user?page=${page}&limit=10`,
            ...
            // Only set error if it's not a 404 or empty result
            if (error.response?.status === 404 || error.response?.data?.message?.includes('No orders found')) {
                // Treat as empty orders, not an error
                setOrders([]);
                setHasMore(false);
            } else {
    ...
    const toggleFavorite = (event, orderId) => {
        ...
        axios.put(
            `${import.meta.env.VITE_SERVER_URL}/api/orderFav/${orderId}`,
            {},
            {
                withCredentials: true,
            }
    ...
    return (
        <div className="min-h-screen flex justify-center items-start">
            <div className="w-full max-w-8xl px-6">
                <h1 className="text-3xl font-extrabold text-gray-900 mb-8 text-center border-b pb-4">Your Tiffin Orders</h1>

                {initialLoad && loading && (
                    <div className="text-center py-10 text-gray-600 text-lg">
                        <p className="flex items-center justify-center animate-pulse"><ClockIcon className="mr-2" size={24} /> Loading your orders...</p>
                        <p className="text-sm mt-2">Hang tight, we're fetching your history!</p>
                    </div>
                )}

                {!initialLoad && error && (
                    <div className="bg-red-50 border border-red-400 text-red-700 px-4 py-3 rounded-md relative text-center text-lg mb-6">
                        <p>{error}</p>
                        <p className="text-sm mt-1">Please refresh the page or try again later.</p>
                    </div>
                )}

                {!initialLoad && !loading && orders.length === 0 && !error && (
                                ...
                                <div className="p-4 flex flex-col items-center text-center">
                                    <div className="relative w-full mb-3">
                                        <img
                                            ...
                                            onError={(e) => { e.target.onerror = null; e.target.src = "https://placehold.co/150x150/e0f2f7/00796b?text=Tiffin+Image"; }}
                                        />
                                        <span
                                        ...
                                        >
                                            ...
                                            {order.status === 'ready' ? "Plan Complete" : order.status}
                                        </span>
                                    ...
                                    <h3 className="text-xl font-bold text-gray-800 mb-1 leading-tight">{tiffinName}</h3>
                                    <p className="text-sm text-gray-600 mb-3">{mealType}</p>

                                    <div className="flex justify-between items-center w-full pt-3 border-t border-gray-100">
                                        <span className="text-xl font-bold text-gray-800">
                                            ₹{order?.totalPrice?.toFixed(2)}
                                        </span>
                                        <button className="px-4 py-2 bg-red-500 text-white rounded-full text-sm font-semibold hover:bg-red-600 transition-colors duration-200 shadow-md">
                                            View Details
                                        </button>
                                    </div>
                                </div>
                ...
                {!hasMore && !loading && orders.length > 0 && (
                    <div className="text-center py-6 text-gray-500 text-sm">You've seen all your tiffin orders.</div>
                )}
            </div>
                ...
                {selectedOrder && (
                    <div className="p-6 bg-white rounded-lg relative">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-700 text-sm mb-6">
                            <div>
                                <p className="font-semibold text-gray-600 flex items-center"><TruckIcon className="mr-2 text-blue-500" size={18} /> Tiffin Provider:</p>
                                <p className="ml-5">{selectedOrder.items[0]?.sourceEntityId?.kitchenName || selectedOrder.items[0]?.name || "N/A"}</p>
                            </div>
                            <div>
                                <p className="font-semibold text-gray-600 flex items-center"><TagIcon className="mr-2 text-purple-500" size={18} /> Meal Type:</p>
                                <p className="ml-5">{selectedOrder.items[0]?.mealType?.name || "N/A"}</p>
                            </div>
                            <div>
                            ...
                            <div>
                                <p className="font-semibold text-gray-600 flex items-center"><ClockIcon className="mr-2 text-green-500" size={18} /> Scheduled Delivery:</p>
                                <p className="ml-5">{(selectedOrder.deliverTime)}</p>
                            </div>
                            <div className="col-span-full">
                                <p className="font-semibold text-gray-600 flex items-center"><MapMarkerIcon className="mr-2 text-red-500" size={18} /> Delivery Address:</p>
                                <p className="ml-5">{selectedOrder.address || "N/A"}</p>
                            </div>
                            <div className="col-span-full">
                                <p className="font-semibold text-gray-600 flex items-center"><PhoneIcon className="mr-2 text-indigo-500" size={18} /> Contact Number:</p>
                                <p className="ml-5">{selectedOrder.phone?.countryCode}{selectedOrder.phone?.number || "N/A"}</p>
                            </div>
                                ...
                                {selectedOrder.items.map((item, idx) => (
                                    <div key={idx} className="flex justify-between items-center bg-gray-50 p-3 rounded-md shadow-sm">
                                        <div className="flex-grow">
                                            ...
                                            {item.mealType && <p className="text-xs text-gray-500">Meal Type: {item.mealType?.name.replace('Meal type: ', '')}</p>}
                                            {item.selectedPlan && <p className="text-xs text-gray-500">Plan: {item.selectedPlan?.name} Days</p>}
                                        </div>
                        ...
                        selectedOrder.status !== 'ready' &&
                         ...
                         selectedOrder.status !== 'user_canceled' && (
